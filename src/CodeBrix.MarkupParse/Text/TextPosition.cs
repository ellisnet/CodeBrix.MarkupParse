...
public readonly struct TextPosition : IEquatable<TextPosition>, IComparable<TextPosition>
{
    ...
    public TextPosition Shift(int columns) => new(_line, (ushort)(_column + columns), _position + columns);
    ...
    /// </returns>
    public override int GetHashCode() => _position ^ (_line | _column) + _line;
    ...
    /// </returns>
    public bool Equals(TextPosition other) => _position == other._position &&
    ...
    /// less, 0 = equal).
    /// </returns>
    public int CompareTo(TextPosition other) => Equals(other) ? 0 : (this > other ? 1 : -1);
