    ...
    where TDocument : class, IConstructableDocument
{
    ...
    public HtmlDomBuilder(
        ...
        bool emitWhitespaceTextNodes = false,
        Func<IConstructableElement, bool> shouldEnd = null)
    {
    ...
    /// </summary>
    public bool IsFragmentCase => _fragmentContext is not null;
    ...
    private bool InColumnGroupEndTagColgroup(ref StructHtmlToken token)
    {
    ...
    /// <param name="tag">The actual tag given.</param>
    /// <returns>True if the token was not ignored, otherwise false.</returns>
    private bool InBodyEndTagBlock(ref StructHtmlToken tag)
    {
    ...
    /// <param name="token">The actual tag token.</param>
    /// <returns>True if the token was not ignored, otherwise false.</returns>
    private bool InBodyEndTagBody(ref StructHtmlToken token)
    {
    ...
    private bool InBodyEndTagParagraph(ref StructHtmlToken token)
    {
        if (IsInButtonScope())
        {
    ...
    /// <param name="token">The actual tag token.</param>
    /// <returns>True if the token was not ignored, otherwise false.</returns>
    private bool InTableEndTagTable(ref StructHtmlToken token)
    {
    ...
    /// <param name="token">The actual tag token.</param>
    /// <returns>True if the token was not ignored, otherwise false.</returns>
    private bool InRowEndTagTablerow(ref StructHtmlToken token)
    {
    ...
    /// <param name="token">The actual tag token.</param>
    /// <returns>True if the token was not ignored, otherwise false.</returns>
    private bool InCaptionEndTagCaption(ref StructHtmlToken token)
    {
    ...
    /// </summary>
    /// <returns>True if it is in scope, otherwise false.</returns>
    private bool IsInScope(HashSet<StringOrMemory> tags)
    {
        for (var i = _openElements.Count - 1; i >= 0; i--)
        {
            var node = _openElements[i];
    ...
    /// </summary>
    /// <returns>True if it is in scope, otherwise false.</returns>
    private bool IsInTableScope(HashSet<StringOrMemory> tags)
    {
        for (var i = _openElements.Count - 1; i >= 0; i--)
        {
            var node = _openElements[i];
    ...
    /// </summary>
    private bool IsCustomElementEverywhere(StringOrMemory tagName) => _options.IsAcceptingCustomElementsEverywhere && tagName.IsCustomElement();
    ...
    private void SetupElement(IConstructableElement element, ref StructHtmlToken tag, bool acknowledgeSelfClosing)
    {
    ...
    /// <param name="tag">The associated tag token.</param>
    /// <param name="acknowledgeSelfClosing">Should the self-closing be acknowledged?</param>
    private TElement AddElement(ref StructHtmlToken tag, bool acknowledgeSelfClosing = false)
    {
    ...
    /// <param name="tag">The associated tag token.</param>
    /// <param name="acknowledgeSelfClosing">Should the self-closing be acknowledged?</param>
    private void AddElement(IConstructableElement element, ref StructHtmlToken tag, bool acknowledgeSelfClosing = false)
    {
        SetupElement(element, ref tag, acknowledgeSelfClosing);
        AddElement(element);
    }
