...
/// </summary>
sealed class CssSelectorConstructor(CssTokenizer tokenizer, IAttributeSelectorFactory attributeSelector, IPseudoClassSelectorFactory pseudoClassSelector, IPseudoElementSelectorFactory pseudoElementSelector, bool invoked = false, bool forgiving = false)
{
    #region Fields
    ...
    private bool _attrInsensitive = false;
    ...
    private bool _invoked = invoked;
    private bool _forgiving = forgiving;

    #endregion

    #region Properties

    public bool IsValid => _invoked && _valid && _ready;
    ...
    private void OnDelim(CssSelectorToken token)
    {
        switch (token.Data[0])
        {
            ...
            case Symbols.Ampersand:
                ...
                // CodeBrix.MarkupParse.Css without requiring CodeBrix.MarkupParse.Css to use 1.1.0
                // of CodeBrix.MarkupParse; instead this architecture also works (without
                // nested support) for CodeBrix.MarkupParse.Css with CodeBrix.MarkupParse 1.0.0.
    ...
    private CssSelectorConstructor CreateChild(bool forgiving) => new(_tokenizer, _attributeSelector, _pseudoClassSelector, _pseudoElementSelector, true, forgiving);
    ...
    {
        public bool Finished(CssSelectorToken token)
        {
    ...
    private sealed class HasFunctionState(CssSelectorConstructor parent) : FunctionState
    {
        ...
        private bool _matchSiblings = false;
    ...
    private sealed class ChildFunctionState(Func<int, int, ISelector, ISelector> creator, CssSelectorConstructor parent, bool withOptionalSelector = true) : FunctionState
    {
        ...
        private readonly bool _allowOf = withOptionalSelector;
        private readonly Func<int, int, ISelector, ISelector> _creator = creator;
        ...
        private bool OnAfterInitialSign(CssSelectorToken token)
        {
            ...
            if (token.Type == CssTokenType.Dimension)
            {
                ...
                _valid = _valid && token.Data.EndsWith("n", StringComparison.OrdinalIgnoreCase) && int.TryParse(integral, NumberStyles.Integer, CultureInfo.InvariantCulture, out _step);
                _step *= _sign;
                _sign = 1;
        ...
        private bool OnOffset(CssSelectorToken token)
        {
            ...
            if (token.Type == CssTokenType.Number)
            {
                _valid = _valid && int.TryParse(token.Data, NumberStyles.Integer, CultureInfo.InvariantCulture, out _offset);
                _offset *= _sign;
