...
public class DOMTests
{
    ...
    public async Task ClosingSpanTagShouldNotResultInAnError()
    {
        ...
        await context.OpenAsync(res => res.Content(source), TestContext.Current.CancellationToken);
    ...
    public void AppendMultipleNodesToParentNode()
    {
        ...
        Assert.Equal(2, document.Body.ChildNodes.Length);
        var span = document.Body.ChildNodes[0];
        var em = document.Body.ChildNodes[1];
        Assert.Equal("span", span.GetTagName());
        Assert.Empty(span.ChildNodes);
        Assert.Equal("em", em.GetTagName());
        Assert.Empty(em.ChildNodes);
    }
    ...
    public void ReadAttributesCorrectlyInAttributeMadness()
    {
        ...
        Assert.Equal(18, attributes.Length);

        for (var i = 0; i < attributes.Length; i++)
        {
            Assert.Equal(expected[i], attributes[i].Name);
        }
    }
    ...
    public void DOMTokenListWritesBack()
    {
        ...
        Assert.Equal(testClass, div.ClassName);
    }
    ...
    public void DOMTokenListCorrectlyInitializedFindsClass()
    {
        ...
        Assert.True(div.ClassList.Contains(testClass));
    }
    ...
    public void DOMTokenListCorrectlyInitializedNoClass()
    {
        ...
        Assert.False(div.ClassList.Contains(testClass));
    }
    ...
    public void DOMTokenListToggleWorksTurnOff()
    {
        ...
        Assert.Equal(div.ClassName, otherClasses);
    }
    ...
    public void DOMTokenListToggleWorksTurnOn()
    {
        ...
        div.ClassList.Toggle(testClass);
        Assert.Equal(div.ClassName, otherClasses + " " + testClass);
    }
    ...
    public void DOMStringMapBindingGet()
    {
        ...
        Assert.Equal(div.Dataset["user"], value);
    }
    ...
    public void DOMStringMapBindingSet()
    {
        ...
        Assert.Equal(div.GetAttribute("data-user"), value);
    }
    ...
    public void DOMStringMapHasNoAttribute()
    {
        var document = new HtmlDocument();
        var div = new HtmlDivElement(document);
        Assert.True(div.Dataset["user"] == null);
    }
    ...
    public void DOMStringMapIEnumerableWorking()
    {
        ...
        div.SetAttribute("data-another", "");
        div.SetAttribute("data-test", "third attribute");
        Assert.Equal(3, div.Dataset.Count());
        Assert.Equal("some", div.Dataset.First().Key);
        Assert.Equal("test", div.Dataset.First().Value);
        Assert.Equal("test", div.Dataset.Last().Key);
        Assert.Equal("third attribute", div.Dataset.Last().Value);
    }
    ...
    public void NormalizeRemovesEmptyTextNodes()
    {
        ...
        div.Normalize();
        Assert.Equal(4, div.ChildNodes.Length);
    }
    ...
    public void NormalizeRemovesEmptyTextNodesNested()
    {
        ...
        div.AppendChild(document.CreateElement("img"));
        div.Normalize();
        Assert.Equal(1, a.ChildNodes.Length);
    }
    ...
    public void LocationCorrectAddressWithoutPort()
    {
        ...
        Assert.Equal(string.Empty, location.Hash);
        Assert.Equal(hostname, location.Host);
        Assert.Equal(hostname, location.HostName);
        Assert.Equal(address, location.Href);
        Assert.Equal(path, location.PathName);
        Assert.Equal(string.Empty, location.Port);
    ...
    public void LocationCorrectAddressWithoutPortButWithHash()
    {
        ...
        var location = new Location(address);
        Assert.Equal(hash, location.Hash);
        Assert.Equal(hostname, location.Host);
        Assert.Equal(hostname, location.HostName);
        Assert.Equal(address, location.Href);
        Assert.Equal(path, location.PathName);
    ...
    public void HtmlAnchorToggleProperties()
    {
        ...
        Assert.True(element.IsTranslated);
        Assert.False(element.IsSpellChecked);
        Assert.False(element.IsContentEditable);
        Assert.False(element.IsDraggable);
        ...
        Assert.False(element.IsTranslated);
        Assert.True(element.IsSpellChecked);
        Assert.True(element.IsDraggable);
        Assert.True(element.IsHidden);
    }
    ...
    public void HtmlButtonToggleProperties()
    {
        ...
        Assert.False(element.IsDisabled);
        Assert.False(element.IsDraggable);
        Assert.Equal(string.Empty, element.FormMethod);
        ...
        Assert.True(element.IsDraggable);
        Assert.Equal("get", element.FormMethod);
    }
...
<form method=""get"" action=""action1"">
    ...
    <input type=""submit"" formmethod=""post"" formaction=""action3"" formenctype=""multipart/form-data"" formtarget=""_blank"">
        ...
        Assert.Equal("get", form.Method);
        Assert.Equal("action1", form.Action);
        Assert.Equal("application/x-www-form-urlencoded", form.Enctype);
        Assert.False(form.NoValidate);
        Assert.Equal(string.Empty, form.Target);

        var button = form.QuerySelector("button") as IHtmlButtonElement;
        Assert.Equal("dialog", button.FormMethod);
        Assert.Equal("action2", button.FormAction);
        Assert.Equal(string.Empty, button.FormEncType);
        Assert.True(button.FormNoValidate);
        Assert.Equal(string.Empty, button.FormTarget);

        var input = form.QuerySelector("input") as IHtmlInputElement;
        Assert.Equal("post", input.FormMethod);
        Assert.Equal("action3", input.FormAction);
        Assert.Equal("multipart/form-data", input.FormEncType);
        Assert.False(input.FormNoValidate);
        Assert.Equal("_blank", input.FormTarget);
    }
...
<!--[if lt IE 9]><script src=""//html5shim.googlecode.com/svn/trunk/html5.js""></script><![endif]-->
        ...
        Assert.Equal(2, html.ChildNodes.Length);
        Assert.Equal(2, html.Attributes.Count());
        Assert.Equal(NodeType.Element, html.NodeType);
        Assert.Equal(@"html", html.GetTagName());

        var head = doc.Head;
        Assert.Equal(19, head.ChildNodes.Length);
        Assert.Empty(head.Attributes);
...
<meta name=""viewport"" content=""width=device-width, initial-scale=1"" />
        ...
        Assert.Matches("<meta property=\\\"og:image\\\" content=\\\"image1.jpg\\\">", resultHtml);
        Assert.Matches("<meta name=\\\"viewport\\\" content=\\\"changed\\\">", resultHtml);
    }
    ...
    public void HtmlCharacterSerialization()
    {
        ...
        var body = doc.Body;
        Assert.NotNull(body);
        Assert.Equal(string.Empty, body.InnerHtml);
...
<!--EndFragment-->
        ...
        Assert.Equal("RU", body.Language);
        Assert.Equal("тест", span.TextContent);
        Assert.Equal("EN-US", span.GetAttribute("lang"));
        Assert.Equal("EN-US", span.Language);
        Assert.Equal("mso-ansi-language:EN-US", span.GetAttribute("style"));
    }

    [Fact]
    public void HtmlWithLangAttributeFromStream()
    {
        var fs = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("CodeBrix.MarkupParse.Tests.Pages.encoding.html");
    ...
    public void TitleRemovalAndAssignment()
    {
        ...
        document.Title = "";
        Assert.Equal("", document.Title);
        Assert.IsAssignableFrom<HtmlTitleElement>(head.LastChild);
        Assert.Null(head.LastChild.FirstChild);
    }
    ...
    public void HeadDuplicatedAndInserted()
    {
        ...
        Assert.Equal(head2, document.Head);
    }
    ...
    public void CreateMarkElementIsNotUnknown()
    {
        ...
        Assert.Equal("MARK", mark.TagName);
        Assert.IsAssignableFrom<IHtmlElement>(mark);
        Assert.IsNotAssignableFrom<IHtmlUnknownElement>(mark);
    }
    ...
    public void CreateDefineElementIsNotUnknown()
    {
        ...
        Assert.Equal("DFN", dfn.TagName);
        Assert.IsAssignableFrom<IHtmlElement>(dfn);
        Assert.IsNotAssignableFrom<IHtmlUnknownElement>(dfn);
    }
    ...
    public void CreateKeyboardElementIsNotUnknown()
    {
        ...
        Assert.Equal("KBD", kbd.TagName);
        Assert.IsAssignableFrom<IHtmlElement>(kbd);
        Assert.IsNotAssignableFrom<IHtmlUnknownElement>(kbd);
    }
    ...
    public void CreateBdoElementIsNotUnknown()
    {
        ...
        Assert.Equal("BDO", bdo.TagName);
        Assert.IsAssignableFrom<IHtmlElement>(bdo);
        Assert.IsNotAssignableFrom<IHtmlUnknownElement>(bdo);
    }
    ...
    public void CreateSpanElementIsNotUnknown()
    {
        ...
        Assert.IsAssignableFrom<IHtmlElement>(span);
        Assert.IsNotAssignableFrom<IHtmlUnknownElement>(span);
    }
    ...
    public void CreateProgressElementIsNotUnknown()
    {
        ...
        Assert.Equal("PROGRESS", progress.TagName);
        Assert.IsAssignableFrom<IHtmlElement>(progress);
        Assert.IsNotAssignableFrom<IHtmlUnknownElement>(progress);
    }
    ...
    public void CreateTimeElementIsNotUnknown()
    {
        ...
        Assert.Equal("TIME", time.TagName);
        Assert.IsAssignableFrom<IHtmlElement>(time);
        Assert.IsNotAssignableFrom<IHtmlUnknownElement>(time);
    }
    ...
    public void Base64ImageTestIsChanged()
    {
        ...
        Assert.Equal(data, img.GetAttribute("src"));
        Assert.Equal(data, ((IHtmlImageElement)img).Source);
    }
    ...
    public void MultipleXmlns_Issue760()
    {
        ...
        Assert.Equal("<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\"></svg>", svg.ToHtml());
    }
    ...
    public void OwnerElement_ShouldNotBeNull()
    {
        ...
        Assert.Same(div, attr.OwnerElement);
    }
    ...
    public void SettingInnerHtmlOnNoScript_ShouldWork()
    {
        ...
        Assert.Equal("<body><noscript>Hello World</noscript></body>", document.Body.ToHtml());
    }
}
