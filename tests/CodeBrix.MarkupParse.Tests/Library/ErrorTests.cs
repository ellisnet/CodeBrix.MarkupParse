...
<article class=""grid-item normal even"">
    ...
    <div class=""grid-abstract"">Bored of JavaScript&#39;s slow evolution? Maybe its time to try Spider.</div>
        ...
        await context.OpenAsync(m => m.Content(source), TestContext.Current.CancellationToken);

        Assert.Single(parseErrors.Received);
        Assert.Equal((int)HtmlParseError.DoctypeMissing, parseErrors.Received[0].Code);
        Assert.Equal(1, parseErrors.Received[0].Position.Column);
        Assert.Equal(1, parseErrors.Received[0].Position.Line);
    }
    ...
    public async Task CallbackOnAnEmptyPageWorksDuringLoading()
    {
        ...
        await context.OpenAsync(m => m.Content(html).Address("http://example.com"), TestContext.Current.CancellationToken);
        Assert.Equal("http://example.com/foo", context.Active.Location.Href);
    }
...
<body>
    <div myAttribute=""blabla>123</div>
        ...
        var parser = new HtmlParser(options);
        Assert.ThrowsAny<HtmlParseException>(() => parser.ParseDocument(html));
    }
}
