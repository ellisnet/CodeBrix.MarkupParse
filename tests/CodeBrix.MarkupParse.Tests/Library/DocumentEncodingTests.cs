...
public class DocumentEncodingTests
{
    ...
    public void BaiduEncodingTransformationEnToUtf8VsZhToUtf8()
    {
        ...
        Assert.Equal("百度一下，你就知道", titleWithEnUs);
        Assert.Equal(titleWithEnUs, titleWithZhCn);
    }
    ...
    public void JdEncodingDisplayCharacters()
    {
        ...
        Assert.Equal("《金庸作品集（21-25）：天龙八部（套装全5册）（朗声旧版）》(金庸)【摘要 书评 试读】- 京东图书", titleWithEnUs);
        Assert.Equal(titleWithEnUs, titleWithZhCn);
    }
    ...
    public void TradeEncodingDisplayCharactersFromWindows1252()
    {
        ...
        Assert.Equal("【单场胜负过关】投注|合买|代购_体彩单场胜负过关游戏_500彩票网", doc.Title);
        Assert.Equal("足球", tr.GetAttribute("matchtype"));
        Assert.Equal("足球-欧罗巴", tr.GetAttribute("lg"));
        Assert.Equal("布鲁日", tr.GetAttribute("homesxname"));
        Assert.Equal("剩余时间", ct.GetAttribute("title"));
        Assert.Equal("克卢日", ni.GetAttribute("homesxname"));
    }
    ...
    public void TradeEncodingVariousChecks()
    {
        ...
        foreach (var e in el)
        {
            ...
            foreach (var tr in t.Rows)
            {
                ...
                Assert.False(string.IsNullOrEmpty(homesxname), "Home team name should exist.");
                Assert.False(homesxname.Contains("?"), "Invalid home team name: " + homesxname);
                Assert.False(string.IsNullOrEmpty(awaysxname), "Away team name should exist.");
                Assert.False(awaysxname.Contains("?"), "Invalid away team name: " + awaysxname);
            }
        }
    }
    ...
    public void EncodingCheckUtf8TestPage()
    {
        ...
        var text = "  ¥ · £ · € · $ · ¢ · ₡ · ₢ · ₣ · ₤ · ₥ · ₦ · ₧ · ₨ · ₩ · ₪ · ₫ · ₭ · ₮ · ₯ · ₹";
        Assert.Equal(text, doc.QuerySelector("big > big").TextContent);
    }
    ...
    public void EncodingCheckWindows1252TestPage()
    {
        ...
        Assert.Equal(text, doc.QuerySelectorAll("tr > td")[7].TextContent);
    }
    ...
    public void EncodingCheckWindows1251TestPage()
    {
        ...
        Assert.Equal(text, doc.QuerySelector("h2").TextContent);
    }
    ...
    public void EncodingCheckBomSaysUTF8ButMetaSaysShiftJisTestPage()
    {
        ...
        var doc = source.ToHtmlDocument();
        var text = "九州大学言語学研究室";
        Assert.Equal(text, doc.QuerySelector("h1").TextContent);
    }
    ...
    public void EncodingCheckRealShiftJisTestPage()
    {
        ...
        var text = "検索";
        Assert.Equal(text, doc.QuerySelector(".form_button").GetAttribute("alt"));
    }
    ...
    public void EncodingCheckGb2312TestPage()
    {
        ...
        var doc = source.ToHtmlDocument();
        var text = "汉字";
        Assert.Equal(text, doc.QuerySelector("td > span").TextContent);
    }
    ...
    public void EncodingCheckGb18030TestPage()
    {
        ...
        var doc = source.ToHtmlDocument();
        var text = "hello，欢迎回来！";
        Assert.Equal(text, doc.QuerySelector(".pptitle > b").TextContent);
    }
    ...
    public void EncodingCheckBig5TestPage()
    {
        ...
        var doc = source.ToHtmlDocument();
        var text = "歷史新聞";
        Assert.Equal(text, doc.QuerySelector("#area_sort_3 a").TextContent);
    }
    ...
    public void EncodingCheckIso88591TestPage()
    {
        ...
        Assert.Equal(text, doc.QuerySelectorAll(".label")[3].TextContent);
    }

    [Fact(Skip = "empireaerials.net is having network issues")]
    ...
    public async Task EncodingFromHeaderAtAmazonFranceSubpageWithConflictingEofCharacter()
    {
        ...
        var document = await BrowsingContext.New(config).OpenAsync(res =>
            res.Content(content).
                ...
                Header(HeaderNames.ContentType, "text/html; charset=ISO-8859-15"), TestContext.Current.CancellationToken);
        var div = document.QuerySelector("#nav-prime-tooltip .nav-npt-text-title");

        Assert.NotNull(div);
        Assert.Equal(" Livraison en 1 jour ouvré gratuite et illimitée sur des millions darticles ", div.TextContent);
    }
    ...
    public async Task EncodingFromHeaderShouldHaveHigherPrecedence()
    {
        ...
        var document = await BrowsingContext.New(config).OpenAsync(res =>
            res.Content(content).
                Header(HeaderNames.ContentType, "text/html; charset=windows-1252"), TestContext.Current.CancellationToken);

        Assert.Equal("windows-1252", document.CharacterSet);
        Assert.Equal("î", document.Body.TextContent);
    }
    ...
    public async Task EncodingFromBomShouldHaveHighestPrecedence()
    {
        ...
        Assert.Equal("utf-8", document.CharacterSet);
        Assert.Equal("î", document.Body.TextContent);
    }
    ...
    public async Task EncodingCanWorkWithUnicodeNonCharacter()
    {
        ...
        var document = await BrowsingContext.New(config).OpenAsync(res =>
            res.Content(content).
                Header(HeaderNames.ContentType, "text/html; charset=utf-8"), TestContext.Current.CancellationToken);
