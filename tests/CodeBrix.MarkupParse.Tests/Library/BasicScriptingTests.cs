...
public class BasicScriptingTests
{
    ...
    public void DocumentWriteDynamicallyWithCustomScriptEngineAndText()
    {
        ...
        {
            Assert.Skip("Prefetched text source is read only");
        }
        ...
        Assert.NotNull(document.Body.TextContent);
        Assert.Equal("//...Dynamically written", document.Body.TextContent);
        Assert.Single(document.QuerySelectorAll("b"));
        Assert.Equal("Dynamically written", bold.TextContent);
    }
    ...
    public async Task DocumentWriteDynamicallyWithCustomScriptEngineAndSource()
    {
        ...
        var document = await BrowsingContext.New(config).OpenAsync(m => m.Content(source).Address(baseAddress), TestContext.Current.CancellationToken);
        var bold = document.QuerySelector("b");
    ...
    public async Task DocumentWriteDynamicallyWithCustomScriptEngineAndSourceNested()
    {
        ...
        var document = await BrowsingContext.New(config).OpenAsync(m => m.Content(source).Address("http://www.example.com"), TestContext.Current.CancellationToken);
        var bold = document.QuerySelector("b");
    ...
    public async Task DocumentWriteConsecutiveWithCustomScriptEngine()
    {
        ...
        Assert.Equal("foobar", document.Body.TextContent);
    }
    ...
    public async Task DocumentWriteDynamicallyWithCustomScriptEngineAndSourceMultipleNested()
    {
        ...
        Assert.Equal("This is dynamically written", document.Body.TextContent);
        Assert.Single(document.QuerySelectorAll("b"));
        Assert.Equal("dynamically written", bold.TextContent);
    ...
    public async Task DocumentLoadExternalJavaScriptJqueryFromDifferentDomain()
    {
        if (Helper.IsNetworkAvailable())
        {
            ...
            Assert.StartsWith("/*! jQuery v2.1.4 | (c) 2005, 2015 jQuery Foundation, Inc. | jquery.org/license */", engine.Requests[0].Item1);
            Assert.Equal(document, engine.Requests[0].Item2.Document);
            Assert.Equal(Encoding.UTF8.WebName, engine.Requests[0].Item2.Encoding.WebName);
        }
    }
