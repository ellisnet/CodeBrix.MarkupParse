...
{
    private static IDocument Html(string code, bool defer = false)
    {
    ...
    public void ConnectMutationObserverAttributesTriggerManually()
    {
        ...
        var observer = new MutationObserver((mut, _) =>
        {
            called = true;
            Assert.Single(mut);
            Assert.Equal(attrName, mut[0].AttributeName);
            Assert.Null(mut[0].PreviousValue);
        });
    ...
    public void ConnectMutationObserverTextWithDescendantsAndClearOldValueTriggerManually()
    {
        ...
        var observer = new MutationObserver((mut, _) =>
        {
            ...
            Assert.NotNull(tn);
            Assert.Equal(text + replaced, tn.TextContent);
        });
    ...
    public void ConnectMutationObserverTextWithDescendantsAndExaminingOldValueTriggerManually()
    {
        ...
        var observer = new MutationObserver((mut, _) =>
        {
            called = true;
            Assert.Single(mut);
            Assert.Equal(text, mut[0].PreviousValue);
            var tn = mut[0].Target as TextNode;
            Assert.NotNull(tn);
            Assert.Equal(text + replaced, tn.TextContent);
        });
    ...
    public void ConnectMutationObserverTextNoDescendantsButCreatedTriggerManually()
    {
        ...
        var observer = new MutationObserver((mut, _) =>
        {
            called = true;
            Assert.Single(mut);
            Assert.Single(mut[0].Added);
            Assert.Equal(text, mut[0].Added[0].TextContent);
        });
    ...
    private static void AssertAll(IMutationRecord[] actualRecords, TestMutationRecord expected)
    {
        ...
        {
            Assert.Equal(expected.Type, actualRecord.Type);
            Assert.Equal(expected.Target, actualRecord.Target);
        }
    }
    ...
    private static void AssertRecord(IMutationRecord actual, TestMutationRecord expected)
    {
        ...
        Assert.Equal(expected.AttributeNamespace, actual.AttributeNamespace);
        Assert.Equal(expected.NextSibling, actual.NextSibling);
        Assert.Equal(expected.PreviousSibling, actual.PreviousSibling);
        Assert.Equal(expected.PreviousValue, actual.PreviousValue);
