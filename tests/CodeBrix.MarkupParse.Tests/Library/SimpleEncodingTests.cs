...
<style type=""text/css"" media=""all"">");
        Assert.Equal(@"euc-jp", doc.CharacterSet);
    }
...
<!-- (control test - for the other tests to work, this should pass - you may have to set your defaults appropriately) -->");
        Assert.Equal(Win1252Name, doc.CharacterSet);
    }
...
<meta charset=""iso8859-2"">");
        Assert.Equal(ISO8859_2Name, doc.CharacterSet);
    }
...
<!-- Starts with UTF-8 BOM -->");
        Assert.Equal(UTF8Name, doc.CharacterSet);
    }
...
<meta name=""Description"" content=""해피카메니아, 손상차, 사고차, 매각차, 체납차 매각전문"">");
        Assert.Equal(@"euc-kr", doc.CharacterSet);
    }
