...
<html lang=en>
  ...
  <p>The requested URL <code>/error</code> was not found on this server.  <ins>That’s all we know.</ins>";
        ...
        Assert.NotEqual(string.Empty, document.DocumentElement.OuterHtml);
    }
    ...
    public void SimpleDocumentManipulation()
    {
        ...
        Assert.NotEqual(repr2, repr1);
        Assert.DoesNotContain(text, repr1);
        Assert.Contains(text, repr2);
    }
    ...
    public void GettingCertainElements()
    {
        ...
        Assert.Equal(new string [] { "Third item!", "Last item!" }, blueListItemsLinq.Select(item => item.Text()));
        Assert.Equal(new string [] { "Third item!", "Last item!" }, blueListItemsCssSelector.Select(item => item.Text()));
    }
    ...
    public void GettingSingleElements()
    {
        ...
        Assert.Equal("<em> bold <u>and</u> italic </em>", emphasize.ToHtml());
        Assert.Equal(" bold and italic ", emphasize.Text());
        Assert.Equal(" bold <u>and</u> italic ", emphasize.InnerHtml);
        Assert.Equal("<em> bold <u>and</u> italic </em>", emphasize.OuterHtml);
        Assert.Equal(" bold and italic ", emphasize.TextContent);
    }
}
